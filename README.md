...
about all other vehicles around the egocar on the same side of the road is received periodically from the simulator. The state of the egocar as well as the states of the other vehicles are stored in __CarState__ objects and handed over to the BehaviorPlanner. The BehaviorPlanner identifies new possible states based on its current state. For each possible state a trajectory is generated by the TrajectoryGenerator and evaluated using several cost functions. Finally, the trajectory with the minimal cost value is taken and returned to the simualtor. 
