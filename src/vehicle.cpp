...
void Vehicle::update_state(map<int, vector < vector<int> > > predictions) {
  ...
  */
  state = KeepLane; // this is an example of how you change state.
...
void Vehicle::realize_prep_lane_change(map<int, vector<vector<int> > > predictions, string direction)
{
  ...
  if (at_behind.size() > 0)
  {
    ...
    for (auto i = 0; i < at_behind.size(); i++)
    {
    ...
    auto target_vel = nearest_behind[1][1] - nearest_behind[0][1];
    auto delta_v = this->v - target_vel;
    auto delta_s = this->s - nearest_behind[0][1];

    if (delta_v != 0)
    {
      auto time = -2 * delta_s / delta_v;
      int a;
    ...
    {
      auto my_min_acc = max(-this->max_acceleration, -delta_s);
      this->a = my_min_acc;
    }
